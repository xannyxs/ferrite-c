...
iso-only:
	@echo "=== Creating ISO (using existing kernel) ==="
...
.PHONY: all kernel userspace install images iso iso-only run debug debug_bochs test clean fclean re lint
